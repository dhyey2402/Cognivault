  ...
  return (
    <div className={`flex h-screen overflow-hidden text-white bg-slate-950`}>
      ...
      <motion.aside 
        ...
        className={`bg-slate-950/80 backdrop-blur-xl border-r border-white/5 text-slate-300 hidden md:flex md:flex-col relative z-20 shadow-[4px_0_24px_rgba(0,0,0,0.06)]`}
      >
      ...
      <div className="md:hidden flex flex-col w-full h-full relative z-10">
        <header className={`bg-slate-950/80 backdrop-blur-xl border-b border-white/10 h-16 flex items-center justify-between px-4 flex-shrink-0`}>
      ...
      <main className="flex-1 flex flex-col hidden md:flex h-full min-w-0 relative z-10">
        <header className={`h-16 flex items-center justify-between px-8 flex-shrink-0 sticky top-0 z-10 bg-transparent border-b border-white/5`}>
           {/* Dynamic Breadcrumbs could go here based on route */}
           <div className={`flex items-center text-sm font-medium text-slate-400`}>
             Admin <span className={`mx-2 text-slate-600`}>/</span> <span className={`capitalize text-white`}>{location.pathname.split('/').pop() || 'Dashboard'}</span>
           </div>
        </header>
        <div className={`flex-1 p-8 overflow-y-auto overflow-x-hidden custom-scrollbar bg-transparent text-white`}>
          <Outlet />
        </div>
      </main>
